...
$consulta = ListaDeRepcionNotaGeneral([$idventa])->fetchAll(PDO::FETCH_ASSOC);

$pdf = new FPDF('P', 'mm', 'A4');
$pdf->AddPage();
$pdf->SetFont('Courier', 'B', 16);
$pdf->Image('img/fondo.png', 125, 8, 60, 60, 'png');
...
$pdf->SetFont('Courier', 'B', 10);
$pdf->Cell(190, 6, utf8_decode('FECHA: ' . date('d-m-Y', strtotime($consulta[0]['FechaRecepcion']))), 0, 'L');
...
$pdf->MultiCell(150, 5, utf8_decode($consulta[0]['DomicilioFiscal']), 1, 'L');
...
foreach (ListaDeRepcionNotaGeneral([$idventa]) as $row) {
...
if ($consulta[0]['TipoRecepcion'] != 1) {
    ...
    $pdf->Cell(47.5, 5, 'SALDO ABONADO: ' . $DeudaResumen['SaldoAbonado'], 1, 0, 'L');
    $pdf->Cell(47.5, 5, 'DEUDA/EXEDENTE ' . $DeudaResumen['SaldoAbonado'] - $DeudaResumen['SaldoDeudor'], 1, 1, 'L');
    $pdf->Ln(10);
}
